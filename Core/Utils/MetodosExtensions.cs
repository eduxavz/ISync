...
namespace ISync.Core.Utils
{
    public static class MetodosExtensions
    {
        ...
        public static T? InfotechToNullable<T>(this string s) where T : struct
        {
            T? result = new T?();
            try
            {
        ...
        public static string InfotechStringNumeros(this string s)
        {
            ...
            return string.Join("", Regex.Split(s, @"[^\d]"));
        }
        ...
        public static decimal? InfotechArredondar(this decimal? valor, int casasDecimais)
        {
            ...
            return valor.Value.InfotechArredondar(casasDecimais);
        }
